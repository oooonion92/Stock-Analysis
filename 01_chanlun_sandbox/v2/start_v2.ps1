...
if (-not $env:CHANLUN_DATA_DIR) {
    $env:CHANLUN_DATA_DIR = "D:\OneDrive\Stock\details"
}
if (-not (Test-Path -LiteralPath $env:CHANLUN_DATA_DIR)) {
    throw "Chanlun market data directory not found: $env:CHANLUN_DATA_DIR"
}
...
function Invoke-DependencyCommand {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Dependency command failed with exit code $LASTEXITCODE"
        }
    } finally {
        foreach ($name in $proxyNames) {
            $value = $savedProxy[$name]
            if ($null -eq $value) {
                Remove-Item -LiteralPath "Env:$name" -ErrorAction SilentlyContinue
            } else {
...
if (-not (Test-Path -LiteralPath $python)) {
    & $bootstrapPython -m venv --system-site-packages $venv
}
...
if ($installedHash -ne $requirementHash) {
    Invoke-DependencyCommand { & $python -m pip install --disable-pip-version-check -r $requirements }
...
if (-not (Test-Path -LiteralPath (Join-Path $frontend "node_modules"))) {
    ...
    try {
        if (Test-Path -LiteralPath $lockfile) {
            Invoke-DependencyCommand { & $pnpm install --frozen-lockfile }
        } else {
            Invoke-DependencyCommand { & $pnpm install }
        }
...
if ($needsBuild) {
    Push-Location $frontend
    try { Invoke-DependencyCommand { & $pnpm build } } finally { Pop-Location }
}
