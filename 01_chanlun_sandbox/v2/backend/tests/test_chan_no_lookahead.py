...
def test_display_start_does_not_restart_chan_structure(client: TestClient) -> None:
    ...
    assert wide["bars"] == narrow["bars"]
    assert wide["viewport_bar_start"] < narrow["viewport_bar_start"]
    assert wide["viewport_bar_end"] == narrow["viewport_bar_end"]
    assert wide["chan"]["decision"]["summary"] == narrow["chan"]["decision"]["summary"]

    assert narrow["chan"]["decision"]["strokes"] == wide["chan"]["decision"]["strokes"]
