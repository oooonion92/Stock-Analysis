...
class DataService:
    ...
    def sync_symbol(self, symbol: str) -> dict[str, Any]:
        ...
        for level, scale in (("5m", 5), ("30m", 30)):
            incoming = self._fetch_sina_bars(symbol, scale)
            path = self.period_file(symbol, level)
            if path.exists():
            ...
            results.append({"level": level, "rows": len(output), "last_time": quality["end"]})
    ...
    @staticmethod
    def _fetch_sina_bars(symbol: str, scale: int) -> pd.DataFrame:
        url = (
