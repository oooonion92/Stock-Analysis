...
class AnalysisService:
    ...
    def analyze(self, request: AnalyzeRequest) -> AnalysisResponse:
        ...
        context_frame = decision if request.display_level == request.execution_level else higher
        context_chan = decision_chan if request.display_level == request.execution_level else higher_chan
        context_level = request.decision_level if request.display_level == request.execution_level else "1d"
        viewport, _ = self._visible_frame(display_frame, request.start)
        history_start = display_frame.iloc[0]["time"]
        active_chan = self._prepare_chan(execution_chan, history_start, request.include_invalidated)
        decision_view = self._prepare_chan(decision_chan, history_start, request.include_invalidated)
        higher_view = self._prepare_chan(higher_chan, history_start, request.include_invalidated)
        ...
        decision_view["inherited_centers"] = self._inherited_centers(
            execution_chan.get("promoted_centers", []), decision_view.get("centers", []), history_start
        )
        higher_view["inherited_centers"] = self._inherited_centers(
            decision_chan.get("promoted_centers", []), higher_view.get("centers", []), history_start
        )
        indicators = {
            "macd": {
                key: values_list
            ...
            "higher_macd": {
                "level": context_level,
                "times": [pd.Timestamp(value).isoformat() for value in context_frame["time"]],
                **{
                    key: values_list
                    for key, values_list in context_chan["macd"].items()
                },
            },
            "wyckoff": {},
        }
        ...
        response = AnalysisResponse(
            ...
            visible_bar_end=pd.Timestamp(display_frame.iloc[-1]["time"]).to_pydatetime(),
            viewport_bar_start=pd.Timestamp(viewport.iloc[0]["time"]).to_pydatetime(),
            viewport_bar_end=pd.Timestamp(viewport.iloc[-1]["time"]).to_pydatetime(),
            symbol=normalized_symbol,
            stock_name=name,
            ...
            bars=self.data.market_bars(display_frame),
            indicators=indicators,
            radar={
                "version": execution_radar["version"],
                "execution": self._prepare_radar(execution_radar, history_start),
                "decision": self._prepare_radar(decision_radar, history_start),
                "higher": self._prepare_radar(higher_radar, history_start),
            },
